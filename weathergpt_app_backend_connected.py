"""
WeatherGPT — a multilingual weather dashboard with a small rule-based
weather assistant, built with Streamlit and the free Open-Meteo API.

Run:
    streamlit run weathergpt_app_patched.py
"""
...
# Change this if the backend is running on another machine/port.
...
def weather_description(code, language: str):
    ...
    return table.get(int(code), fallback)
...
def ask_weathergpt_backend(
    ...
    language handling -> NLP -> weather retrieval -> risk assessment
    -> SQL advisory -> RAG -> Qwen -> output translation.
    """
    language_code = BACKEND_LANGUAGE_CODES.get(
        language,
        "en",
    )
...
if st.session_state.last_question:
    ...
    with st.chat_message("assistant"):
        ...
        if "error" in backend_result:
            ...
            # Display the actual multilingual WeatherGPT response.
            st.write(
                backend_result.get(
                    "reply",
                    "No response received from WeatherGPT."
                )
            )

            # Show structured information returned by the backend.
            backend_weather = backend_result.get(
                "weather"
            )

            backend_risk = backend_result.get(
                "risk"
            )

            backend_advisory = backend_result.get(
                "advisory"
            )
            ...
            if backend_risk:
                with st.expander("Risk assessment"):
                    st.write(
                        f"**Hazard:** "
                        f"{backend_risk.get('hazard', 'none')}"
                    )
                    st.write(
                        f"**Level:** "
                        f"{backend_risk.get('level', 'low')}"
                    )
                    st.write(
                        f"**Reason:** "
                        f"{backend_risk.get('reason', '')}"
                    )
                    st.write(
                        f"**Recommended action:** "
                        f"{backend_risk.get('recommended_action', '')}"
                    )
            ...
            if backend_advisory:
                with st.expander("Crop advisory"):
                    ...
                    st.write(
                        f"**Hazard:** "
                        f"{backend_advisory.get('hazard', '')}"
                    )
                    st.write(
                        f"**Recommendation:** "
                        f"{backend_advisory.get('recommendation', '')}"
                    )
