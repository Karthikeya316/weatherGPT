...
BACKEND_LANGUAGE_CODES = {"English":"en","Hindi":"hi","Telugu":"te","Tamil":"ta","Kannada":"kn","Malayalam":"ml"}
...
def ask_weathergpt_backend(question: str, city: str, language: str):
    language_code = BACKEND_LANGUAGE_CODES.get(language, "en")
    try:
        response = requests.post(f"{BACKEND_URL}/chat", json={"message": question, "location": city, "language": language_code}, timeout=180)
        response.raise_for_status()
...
if st.session_state.last_question:
    ...
    backend_result = ask_weathergpt_backend(
        st.session_state.last_question, city, language
    )

    with st.chat_message("assistant"):
        if "error" in backend_result:
            st.error("❌ WeatherGPT backend is unavailable. Please make sure the FastAPI server is running.")
            st.caption(f"Backend: {BACKEND_URL}")
        else:
            st.write(backend_result.get("reply", "No response received from WeatherGPT."))
            backend_weather = backend_result.get("weather")
            backend_risk = backend_result.get("risk")
            backend_advisory = backend_result.get("advisory")
            if backend_weather:
                with st.expander("WeatherGPT details"):
                    st.write(f"**Location:** {backend_weather.get('location', city)}")
                    st.write(f"**Date:** {backend_weather.get('date', 'N/A')}")
                    st.write(f"**Temperature:** {backend_weather.get('temp_min_c', 0):.1f}°C – {backend_weather.get('temp_max_c', 0):.1f}°C")
                    st.write(f"**Rainfall:** {backend_weather.get('rainfall_mm', 0):.1f} mm")
                    st.write(f"**Precipitation probability:** {backend_weather.get('precipitation_probability_pct', 0):.0f}%")
                    st.write(f"**Humidity:** {backend_weather.get('humidity_pct', 0):.0f}%")
                    st.write(f"**Wind:** {backend_weather.get('wind_kmph', 0):.1f} km/h")
                    st.write(f"**Condition:** {backend_weather.get('condition', 'Unknown')}")
            if backend_risk:
                with st.expander("Risk assessment"):
                    st.write(f"**Hazard:** {backend_risk.get('hazard', 'none')}")
                    st.write(f"**Level:** {backend_risk.get('level', 'low')}")
                    st.write(f"**Reason:** {backend_risk.get('reason', '')}")
                    st.write(f"**Recommended action:** {backend_risk.get('recommended_action', '')}")
            if backend_advisory:
                with st.expander("Crop advisory"):
                    st.write(f"**Crop:** {backend_advisory.get('crop', '')}")
                    st.write(f"**Hazard:** {backend_advisory.get('hazard', '')}")
                    st.write(f"**Recommendation:** {backend_advisory.get('recommendation', '')}")
